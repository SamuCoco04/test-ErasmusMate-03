...
describe('Social: discovery visibility', () => {
  ...
  it('cancelled outgoing request becomes requestable again for open profiles', async () => {
    ...
    const maria = items.find((x) => x.id === 'sp-student-2');
    expect(maria?.connectionStatus).toBe('AVAILABLE_TO_REQUEST');
    expect(maria?.allowedActions.request).toBe(true);
  });
