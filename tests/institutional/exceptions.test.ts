...
describe('Institutional exceptions workflow', () => {
  it('student can create exception for own deadline and audit is written', async () => {
    const created = await createExceptionRequest({ role: 'STUDENT', userId: 'student-1' }, { title: 'Ask for more time', reason: 'Medical reason', deadlineId: 'deadline-1' });
  ...
  it('coordinator cannot transition exception outside assigned mobility', async () => {
    const created = await createExceptionRequest({ role: 'STUDENT', userId: 'student-1' }, { title: 'Ask for more time', reason: 'Travel issue', deadlineId: 'deadline-1' });
  ...
  it('apply updates deadline override and writes audits', async () => {
    const created = await createExceptionRequest({ role: 'STUDENT', userId: 'student-1' }, { title: 'Ask for more time', reason: 'Embassy delay', deadlineId: 'deadline-1' });
    ...
    const deadline = await prisma.deadline.findUniqueOrThrow({ where: { id: 'deadline-1' } });
