...
describe('Institutional attachment API multipart flow', () => {
  ...
  it('accepts multipart upload and persists file metadata from uploaded file', async () => {
    ...
    form.set(
      'file',
      new File([payload], 'housing-proof.pdf', {
        type: 'application/pdf',
      }),
    );

    const request = new Request(
      'http://localhost/api/institutional/submissions/sub-1/attachments',
      {
        method: 'POST',
        body: form,
      },
    ...
    const response = await addAttachmentRoute(request, {
    ...
    const fromDb = await prisma.documentAttachment.findUniqueOrThrow({
    ...
    expect(fromDb.submissionId).toBe('sub-1');
  });
  ...
  it('replacement marks prior ACTIVE attachment as REPLACED and creates a new ACTIVE version', async () => {
    ...
    form.set(
      'file',
      new File([new Uint8Array([1, 2, 3, 4])], 'transcript-request-v2.pdf', {
        type: 'application/pdf',
      }),
    );

    const request = new Request(
      'http://localhost/api/institutional/submissions/sub-4/attachments/att-4/replace',
      {
        method: 'POST',
        body: form,
      },
    ...
    const response = await replaceAttachmentRoute(request, {
    ...
    expect(json.data.fileName).toBe('transcript-request-v2.pdf');
    expect(json.data.status).toBe('ACTIVE');
    expect(json.data.version).toBe(2);
    expect(json.data.submissionId).toBe('sub-4');

    const oldAttachment = await prisma.documentAttachment.findUniqueOrThrow({
      where: { id: 'att-4' },
    });

    const newAttachment = await prisma.documentAttachment.findUniqueOrThrow({
      where: { id: json.data.id },
    });

    expect(oldAttachment.status).toBe('REPLACED');

    expect(newAttachment.fileName).toBe('transcript-request-v2.pdf');
