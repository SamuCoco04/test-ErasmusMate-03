...
import type { DiscoveryConnectionStatus } from './types';
...
export async function transitionConnection(prisma: PrismaClient, actor: {role:string; userId:string}, connectionId: string, action: string) {
  ...
  if (action === 'accept') {
    if (c.state !== 'PENDING' || c.receiverProfileId !== me.id) throw new SocialInvalidTransitionError();
    return prisma.socialConnection.update({ where: { id: connectionId }, data: { state: 'ACCEPTED', respondedAt: new Date(), lastActionByProfileId: me.id } });
  }
  if (action === 'reject') {
    if (c.state !== 'PENDING' || c.receiverProfileId !== me.id) throw new SocialInvalidTransitionError();
    return prisma.socialConnection.update({ where: { id: connectionId }, data: { state: 'REJECTED', respondedAt: new Date(), lastActionByProfileId: me.id } });
  }
  if (action === 'cancel') {
    if (c.state !== 'PENDING' || c.requesterProfileId !== me.id) throw new SocialInvalidTransitionError();
    return prisma.socialConnection.update({ where: { id: connectionId }, data: { state: 'CANCELLED', respondedAt: new Date(), lastActionByProfileId: me.id } });
  }
  if (action === 'block') {
...
export async function getMyConnections(prisma: PrismaClient, actor: {role:string; userId:string}) {
  ensureStudent(actor.role);
  const me = await requireMyProfile(prisma, actor.userId);
  ...
  const incomingPending = all.filter((x)=>x.state==='PENDING'&&x.receiverProfileId===me.id);
  const outgoingPending = all.filter((x)=>x.state==='PENDING'&&x.requesterProfileId===me.id);
  const accepted = all.filter((x)=>x.state==='ACCEPTED');
  const unavailable = all.filter((x)=>['BLOCKED','REJECTED','CANCELLED'].includes(x.state));
  return { incomingPending, outgoingPending, accepted, unavailable };
}
