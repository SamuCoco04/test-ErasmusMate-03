...
export async function getDeadlineSummary(ctx: DemoContext) { const deadlines: DeadlineReadModel[] = ctx.role === 'STUDENT' ? await listDeadlinesForStudent(ctx) : ctx.role === 'COORDINATOR' ? await listDeadlinesForCoordinator(ctx) : (await prisma.deadline.findMany({ include: { relatedProcedure: true, exceptionRequests: true, mobilityRecord: { include: { student: true } } } })).map((d) => toReadModel(d)); return { counts: { upcoming: deadlines.filter((d) => d.effectiveState === 'UPCOMING').length, overdue: deadlines.filter((d) => d.effectiveState === 'OVERDUE').length, fulfilled: deadlines.filter((d) => d.effectiveState === 'FULFILLED').length, overridden: deadlines.filter((d) => d.effectiveState === 'OVERRIDDEN').length }, items: deadlines }; }
