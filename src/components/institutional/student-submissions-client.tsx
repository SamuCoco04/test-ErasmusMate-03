...
export function StudentSubmissionsClient({ initialSubmissions, procedureOptions }: { initialSubmissions: SubmissionView[]; procedureOptions: { id: string; title: string }[] }) {
  ...
  return <div className='space-y-4'>
    ...
    {initialSubmissions.map((s) => {
      ...
      return <div key={s.id} className='rounded border p-3 text-sm'><div className='font-medium'>{s.procedureTitle}</div><div>Status: {s.state}</div>{s.reviewerNotes && <div>Coordinator notes: {s.reviewerNotes}</div>}<ul className='list-disc pl-5 text-xs'>{s.attachments.map((a) => <li key={a.id}>{a.fileName} · {a.status}{editable && <button className='ml-2 underline' onClick={() => call(`/api/institutional/submissions/${s.id}/attachments/${a.id}/remove`, {})}>Remove</button>}</li>)}</ul>{editable && <button className='mt-2 underline' onClick={() => call(`/api/institutional/submissions/${s.id}/attachments`, { fileName: 'new-upload.pdf', mimeType: 'application/pdf', sizeBytes: 1200, storageKey: `demo/${s.id}/new-upload.pdf` })}>Quick add demo attachment</button>}<div className='mt-2 space-y-1'>{!s.hasActiveAttachment && editable && <div className='text-xs text-amber-700'>Add at least one active attachment before submit or resubmit.</div>}{s.state === 'DRAFT' && <button disabled={!s.hasActiveAttachment} className='underline disabled:cursor-not-allowed disabled:text-slate-400' onClick={() => call(`/api/institutional/submissions/${s.id}/transition`, { action: 'submit' })}>Submit for review</button>}{['REJECTED', 'REOPENED', 'NEEDS_CORRECTION'].includes(s.state) && <button disabled={!s.hasActiveAttachment} className='underline disabled:cursor-not-allowed disabled:text-slate-400' onClick={() => call(`/api/institutional/submissions/${s.id}/transition`, { action: 'resubmit' })}>Resubmit</button>}</div></div>;
    })}
  </div>;
}
