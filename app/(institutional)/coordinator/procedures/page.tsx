...
type Procedure = { id: string; title: string; description: string; isRequired: boolean; sortOrder: number };
...
export default function CoordinatorProceduresPage() {
  ...
  return <div className='space-y-4'><h1 className='text-xl font-semibold'>Procedures</h1>
    <form className='grid gap-2 rounded border p-3 text-sm' onSubmit={async (e) => { e.preventDefault(); const fd = new FormData(e.currentTarget); await fetch('/api/institutional/procedures', { method:'POST', headers:{'content-type':'application/json'}, body: JSON.stringify({ title: String(fd.get('title')??''), description: String(fd.get('description')??''), isRequired: fd.get('isRequired')==='on' })}); await load(); }}>
      ...
      <label><input name='isRequired' type='checkbox' defaultChecked /> Required</label>
      <button className='w-fit rounded bg-blue-600 px-3 py-1 text-white'>Create procedure</button>
    </form>
    {items.map((i)=><div key={i.id} className='rounded border p-2 text-sm'><b>{i.title}</b> · {i.isRequired?'Required':'Optional'}<div>{i.description}</div></div>)}
  </div>;
}
