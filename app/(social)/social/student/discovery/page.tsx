...
const stateLabel: Record<string, string> = { AVAILABLE_TO_REQUEST: 'Available to request', REQUEST_SENT: 'Request sent', REQUEST_RECEIVED: 'Request received', CONNECTED: 'Connected', BLOCKED: 'Blocked', UNAVAILABLE: 'Unavailable' };
...
export default function SocialDiscoveryPage() {
  ...
  return <PageShell>
    ...
    <div className='grid gap-3 md:grid-cols-2'>{items.map((p) => { const canRequest = p.connectionStatus === 'AVAILABLE_TO_REQUEST'; const canMessage = p.connectionStatus === 'CONNECTED'; return <article key={p.id} className='rounded-xl border bg-white p-4'>
