...
async function runSeedOnce() {
  ...
  await prisma.user.upsert({ where: { id: 'student-7' }, update: { email: 'student7@erasmusmate.demo', displayName: 'Tomas Varga', role: 'STUDENT', institutionId: 'inst-home-1' }, create: { id: 'student-7', email: 'student7@erasmusmate.demo', displayName: 'Tomas Varga', role: 'STUDENT', institutionId: 'inst-home-1' } });
  await prisma.user.upsert({ where: { id: 'student-8' }, update: { email: 'student8@erasmusmate.demo', displayName: 'Ana Petrovic', role: 'STUDENT', institutionId: 'inst-home-1' }, create: { id: 'student-8', email: 'student8@erasmusmate.demo', displayName: 'Ana Petrovic', role: 'STUDENT', institutionId: 'inst-home-1' } });
  await prisma.user.upsert({ where: { id: 'student-9' }, update: { email: 'student9@erasmusmate.demo', displayName: 'Jonas Meyer', role: 'STUDENT', institutionId: 'inst-home-1' }, create: { id: 'student-9', email: 'student9@erasmusmate.demo', displayName: 'Jonas Meyer', role: 'STUDENT', institutionId: 'inst-home-1' } });
  await prisma.user.upsert({ where: { id: 'student-10' }, update: { email: 'student10@erasmusmate.demo', displayName: 'Sofia Ionescu', role: 'STUDENT', institutionId: 'inst-home-1' }, create: { id: 'student-10', email: 'student10@erasmusmate.demo', displayName: 'Sofia Ionescu', role: 'STUDENT', institutionId: 'inst-home-1' } });
  ...
  const socialProfiles = [
    ...
    { id:'sp-student-7', userId:'student-7', displayName:'Tomas Varga', homeCity:'Bratislava', hostCity:'Leuven', hostCountry:'Belgium', homeInstitutionName:'Universidad de Sevilla', hostInstitutionName:'KU Leuven', studyArea:'Industrial Engineering', mobilityPhase:'On mobility', languagesJson: JSON.stringify(['English','Slovak']), interestsJson: JSON.stringify(['Board games']), bio:'Looking for Erasmus friends in Leuven.', visibility:'VISIBLE', contactPreference:'OPEN_TO_REQUESTS', mapVisibility:'CITY_ONLY', approximateLatitude:50.8798, approximateLongitude:4.7005, moderationState:'ACTIVE' },
    { id:'sp-student-8', userId:'student-8', displayName:'Ana Petrovic', homeCity:'Zagreb', hostCity:'Leuven', hostCountry:'Belgium', homeInstitutionName:'Universidad de Sevilla', hostInstitutionName:'KU Leuven', studyArea:'Computer Science', mobilityPhase:'Before departure', languagesJson: JSON.stringify(['English','Croatian']), interestsJson: JSON.stringify(['Hackathons']), bio:'Preparing my Leuven semester and open to connect.', visibility:'VISIBLE', contactPreference:'OPEN_TO_REQUESTS', mapVisibility:'CITY_ONLY', approximateLatitude:50.8796, approximateLongitude:4.7011, moderationState:'ACTIVE' },
    { id:'sp-student-9', userId:'student-9', displayName:'Jonas Meyer', homeCity:'Hamburg', hostCity:'Brussels', hostCountry:'Belgium', homeInstitutionName:'Universidad de Sevilla', hostInstitutionName:'VUB', studyArea:'History', mobilityPhase:'On mobility', languagesJson: JSON.stringify(['English','German']), interestsJson: JSON.stringify(['Museums','Cycling']), bio:'Happy to share Brussels practical tips.', visibility:'VISIBLE', contactPreference:'OPEN_TO_REQUESTS', mapVisibility:'CITY_ONLY', approximateLatitude:50.8503, approximateLongitude:4.3517, moderationState:'ACTIVE' },
    { id:'sp-student-10', userId:'student-10', displayName:'Sofia Ionescu', homeCity:'Bucharest', hostCity:'Leuven', hostCountry:'Belgium', homeInstitutionName:'Universidad de Sevilla', hostInstitutionName:'KU Leuven', studyArea:'Psychology', mobilityPhase:'On mobility', languagesJson: JSON.stringify(['English','Romanian']), interestsJson: JSON.stringify(['Student wellbeing']), bio:'', visibility:'VISIBLE', contactPreference:'HIDDEN', mapVisibility:'HIDDEN', approximateLatitude:null, approximateLongitude:null, moderationState:'ACTIVE' },
  ] as const;
  ...
  const socialConnections = [
    ...
    { id: 'conn-seed-5', pairKey: 'sp-student-1__sp-student-7', requesterProfileId: 'sp-student-7', receiverProfileId: 'sp-student-1', state: 'PENDING', requestedAt: new Date('2026-05-02T08:00:00.000Z'), lastActionByProfileId: 'sp-student-7' },
  ] as const;
...
await prisma.mobilityRecord.upsert({ where: { id: 'mobility-1' }, update: { studentId: 'student-1', coordinatorId: 'coordinator-1', homeInstitutionId: 'inst-home-1', hostInstitutionId: 'inst-host-1', mobilityStatus: 'ACTIVE', startDate: new Date('2026-09-01T00:00:00.000Z'), endDate: new Date('2027-01-31T00:00:00.000Z') }, create: { id: 'mobility-1', studentId: 'student-1', coordinatorId: 'coordinator-1', homeInstitutionId: 'inst-home-1', hostInstitutionId: 'inst-host-1', mobilityStatus: 'ACTIVE', startDate: new Date('2026-09-01T00:00:00.000Z'), endDate: new Date('2027-01-31T00:00:00.000Z') } });
  ...
  const documentSubmissions = [
    ...
    { id: 'sub-5', mobilityRecordId: 'mobility-1', procedureId: 'proc-1', state: 'APPROVED', submittedAt: new Date('2026-04-01T09:00:00.000Z'), reviewedAt: new Date('2026-04-03T09:00:00.000Z'), reviewerNotes: null,}, 
  ] as const;
