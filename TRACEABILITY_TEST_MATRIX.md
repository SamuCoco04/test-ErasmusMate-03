...
| E2E-EXC-004 | Exception application | Approved exception can apply supported change | Coordinator | E2E | Coordinator approves and applies a student exception, then student sees the applied outcome (`e2e/institutional/workflows.spec.ts`) | Exception moves to Applied and student-facing exception card reflects applied rationale/state | Implemented - passing E2E |
...
| E2E-LA-010 | Denied-row resubmission | Denied rows must be revised before resubmission | Student | E2E | Student attempts resubmit before revising requested-changes row (`e2e/institutional/workflows.spec.ts`) | Resubmission is blocked with clear message until row is updated | Implemented - passing E2E |
| E2E-LA-011 | Denied-row revision | Student can revise denied row and resubmit | Student | E2E | Coordinator requests changes; student revises denied row and resubmits from My Learning Agreement (`e2e/institutional/workflows.spec.ts`) | Resubmission is blocked until denied row is revised, then agreement returns to review | Implemented - passing E2E |
...
| E2E-NOTIF-001 | Institutional notifications | Recipient sees own notifications and can mark as read | Student / Coordinator | E2E | Coordinator approval triggers student notification; student marks it read and coordinator cannot see student-only notification (`e2e/institutional/workflows.spec.ts`) | Notification visibility remains owner-scoped and read status updates in UI | Implemented - passing E2E |
