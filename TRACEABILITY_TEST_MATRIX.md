...
| SERVICE-CONN-002 | Request lifecycle | Eligible student can send/cancel/re-send request | Student | Service/API | Execute request -> cancel -> request again on same pair (`tests/social/connections.test.ts`) | Pair row is reused and active duplicate pending is prevented | Implemented - service/API tests |
| SERVICE-CONN-003 | Incoming request handling | Receiver can accept or reject incoming request | Student | Service/API | Transition incoming pending to accepted/rejected (`tests/social/connections.test.ts`) | State transitions succeed only for receiver | Implemented - service/API tests |
| SERVICE-CONN-004 | Block/unblock | Connected or pending pair can be blocked/unblocked with message guard | Student | Service/API | Block accepted connection, verify message forbidden, unblock and re-request (`tests/social/connections.test.ts`) | Message blocked while blocked; pair recovers to requestable flow | Implemented - service/API tests |
| UI-CONN-001 | Discovery/Connections controls | Lifecycle controls are visible for demo | Student | Service/API | Static contract check for Send request / Request sent / Cancel request / Accept / Reject / Message / Block / Unblock (`tests/social/social-ui-contracts.test.ts`) | Required controls/copy remain visible and stable | Implemented - UI contract tests |
| UI-CONN-002 | Unavailable privacy state | Privacy-restricted profile shows unavailable reason | Student | Service/API | Contract check for unavailable helper copy (`tests/social/social-ui-contracts.test.ts`) | Profile shows clear unavailable reason and no active request action | Implemented - UI contract tests |
| DEMO-SEED-CONN-001 | Seeded social demo states | Seed provides requestable/pending/incoming/accepted/blocked/unavailable states | Student | Manual | Run seed and inspect discovery/connections pages | Demo can manually validate lifecycle states deterministically | Implemented - seeded demo data |
| E2E-CONN-006 | Discovery lifecycle | Discovery send->cancel->re-send path | Student | E2E | Discovery + connections end-to-end path | Deterministic UI lifecycle execution | Partial - E2E deferred |

| SOC-CONN-8D1-001 | Cancel -> request again mapping | Cancelled outgoing request must return to requestable state when target accepts requests | Student | Service/API | Cancel pending request to Maria-like profile, then discovery state mapping check (`tests/social/discovery.test.ts`) | Status returns to `AVAILABLE_TO_REQUEST` and `Send request` is allowed | Implemented - passing |
| SOC-CONN-8D1-002 | Demo seed lifecycle determinism | Seeded profile used for cancel/re-request demo must be requestable after cancellation | Student | Seed + Service/API | Seed Maria as `OPEN_TO_REQUESTS` while keeping Elena privacy-restricted (`prisma/seed.ts`, social tests) | Demo consistently supports send -> cancel -> send again without fake unavailability | Implemented - passing |
